    ...
    internal sealed class ReagentGrinderSystem : EntitySystem
    {
        ...
        private void OnInteractUsing(Entity<ReagentGrinderComponent> entity, ref InteractUsingEvent args)
        {
            ...
            if (!HasComp<ExtractableComponent>(heldEnt))
            {
                ...
                {
                    var doAfter = new DoAfterArgs(EntityManager, args.User, 1f, new ContainerDoAfterEvent(), entity, entity, used: heldEnt)
                    {
                        BreakOnDamage = true,
                        NeedHand = true,
                        BreakOnMove = true,
        ...
        private void OnContainerDoAfter(Entity<ReagentGrinderComponent> ent, ref ContainerDoAfterEvent args)
        {
            ...
            foreach (var (item, _) in storage.StoredItems)
            {
                ...
                {
                    _popupSystem.PopupEntity(Loc.GetString("reagent-grinder-component-storage-full-message"), ent.Owner, args.User);
                    break;
                }
            ...
            _audioSystem.PlayPvs(ent.Comp.ItemsAddedSound, ent.Owner); //play a sound to indicate completion
            args.Handled = true;
        }
